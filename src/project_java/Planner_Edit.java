...
//		add image 
			        ...
			        if (cnt == 0) {
			            ...
			            for (int i = 0; i < placeTable2.getRowCount(); i++) {
			                try {
			                    ...
			                    p.setCmd(2006);
			                    planner_Select.main.out.writeObject(p);
			                    planner_Select.main.out.flush();
			                    index2 = index;
			                } catch (Exception e2) {
		...
		Travel_Location_VO vo = Travel_Location_DAO.getLocation2(planVo.getTL_NUM());
		// 제목 부분
